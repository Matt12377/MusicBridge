...
## 开放式封面与完整播放栏（2026-09-05）

- Owner 的最新截图反馈明确覆盖旧卡片底板、窄页面、58px 歌曲行和 88px 播放栏尺寸：封面脱离容器底板，内容两侧收紧；歌曲列表使用连续背景、大封面与来源提供的元数据。
- 正式布局在 `open-library.css`，保留浅深色与封面背景机制。主列表行高 84px、队列行高 80px，虚拟滚动常量必须与实际高度一致。
- 播放栏展示当前实际音质，按钮只显示音质值；弹出菜单仍说明偏好应用于后续播放，不能把选择偏好冒充当前实际音质。
- 音量通过所选 Roon Zone 的输出调节，保留设备原生单位、范围和步长；分组输出分别控制。无控制能力时明确提示，不用本机系统音量替代。
- 本轮几何与交互验证入口为 `apps/desktop/scripts/open-library-ui-gate.mjs`；旧 ambient 预览及 Gate 的尺寸对照仅对应之前的背景基线，不再代表最新布局。

## 播放栏交互反馈（2026-09-05 17:40）

- Owner 要求底部进度条收短居中，当前采用最大 360px；网易云歌曲列表暂时移除音质行，保留播放栏的真实音质。
- 进度显示时钟独立于快照轮询，重复位置不重置计时，小误差通过速度校准，不能把整数秒回报直接覆盖成倒退的滑块。拖动确认、失败及切歌必须区分。
- 音量在 input 阶段连续发送，约 80ms 合并最新值、单请求串行；拖动期间不禁用滑块。释放后保持草稿直到设备读数确认，旧回报不得造成闪回；失败和超时需恢复观测并提示。

## UI 里程碑已确认（2026-09-05）

- Owner 明确确认本轮 UI 修改完成一个里程碑，要求将当前产物提交到 UI 基线。以 `reports/V3_UI_MILESTONE_2026-09-05.md` 与标签 `ui-milestone-2026-09-05` 为当前视觉基准；上述各轮“待视觉确认”属于历史记录，由本次确认覆盖。
- 保留连续背景、手动浅深色、播放封面背景、开放封面墙和无容器歌曲列表。播放器最大 1120px，窄窗口居中收窄以避开侧栏；进度区域最大 360px。
- 主导航为主页、专辑、艺术家、流派、收藏、实物收藏、录音、歌单。收藏仅 Roon；歌单仅网易云，采用与主导航一致的图标行并可展开/收起，不再显示两条来源入口。设置固定到底部，留 16px 边距。
- 下一阶段是整个“实物收藏”UI 重制，另立范围并从本里程碑基线继续。先制作独立 HTML 预览，得到具体视觉反馈后迁入正式应用；保留现有库存、参考目录、关联关系、照片和录音业务数据，不把 UI 重制推定为数据迁移或删除授权。
- 本次接受范围是 UI 里程碑；真实设备、音频、录音、发布与历史 carryover 仍按各自证据处理。

## 搜索范围（2026-09-19）

- Owner 已确认聚合搜索页面没有发现问题。主页搜索维持网易云 + Roon 聚合结果；同名专辑保留来源与独立条目，艺人仅在无歧义时聚合展示。
- Owner 要求专辑/艺术家页面提供各自的本地搜索，结果限定当前页面的 Roon 类型，不混入网易云；页面内搜索与主页聚合搜索保持独立。
