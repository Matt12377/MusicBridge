...
- Owner 要求同时保留浅色和蓝灰深色，在设置内自行切换。入口为“设置 → 应用 → 外观 → 主题”，默认继续浅色；选择即时生效、在本机保存，并同步原生窗口主题。
- 两种外观共用布局与封面背景机制，主题切换不改变曲目、队列或播放状态。此次没有增加自动跟随系统。
- `prototypes/ambient-study/` 同样提供浅深色切换；正式应用的视觉确认仍以真实媒体库中的 Owner 反馈为准。
