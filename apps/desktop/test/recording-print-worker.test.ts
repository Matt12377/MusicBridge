...
test('Main在Core就绪时启动打印worker，失效与退出时停止，公开preload接八入口', async () => {
  ...
  assert.match(main, /onReady: async \(\) => \{\s*await startRecordingPrintWorker\(supervisor\)/u)
  assert.match(main, /onLifecycle: \(event\) => \{\s*if \(event.event !== 'ready'\) stopRecordingPrintWorker\(\)/u)
