...
async function mounted(t: test.TestContext, api: c.RecordingRecordsPublicApi, props: { physicalId?: string; draftId?: string } = {}) {
  ...
  const controller = await import('../src/renderer/src/components/recording/recording-record-controller.js')
  ...
  function loadSfc(filename: string): import('vue').Component {
    ...
    const load = (name: string): unknown => name === 'vue' ? vue : name.endsWith('recording-record-controller') ? controller : name.endsWith('.vue') ? { default: loadSfc(path.resolve(path.dirname(filename), name)) } : require(name)
