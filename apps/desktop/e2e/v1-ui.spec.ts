...
test('V3 实物照片原生导入、代表图与重启持久化，不预分配单盘编号', async () => {
  ...
  await expect(page.getByTestId('inventory-total')).toHaveText('5')
  await page.getByRole('region', { name: '实物照片', exact: true }).locator('.collection-photo').first().scrollIntoViewIfNeeded()
...
test('V3 单盘照片拒绝非法文件；加载失败可重试且不丢库存', async () => {
  ...
  const photos = page.getByRole('region', { name: '实物照片', exact: true })
  await photos.locator('.collection-photo').scrollIntoViewIfNeeded()
  await expect(photos.getByText('照片读取失败，可打开大图重试', { exact: true })).toBeVisible()
