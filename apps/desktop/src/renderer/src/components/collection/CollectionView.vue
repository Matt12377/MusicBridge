  ...
  <section class="collection-view" data-component="CollectionView" aria-label="实体收藏">
    ...
    >
      <header class="collection-heading">
        ...
        <button class="collection-add" type="button" disabled :aria-describedby="`collection-status-${view.id}`">
      ...
      <div class="collection-empty">
        ...
        <h3>{{ view.id === 'tapes' ? '这里将放下你的磁带收藏' : '这里将保存你拥有的音乐' }}</h3>
        ...
        <p :id="`collection-status-${view.id}`" class="collection-status">库存录入与照片管理尚未接入，当前不展示示例库存。</p>
      </div>
    </div>
  </section>
</template>
