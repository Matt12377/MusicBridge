...
import { isCanonicalReference, isCatalogMapping, isPreviewCatalogRevisionRequest, type CanonicalReference, type CatalogMatch, type SourcePack } from '@music-bridge/contracts'
...
<template>
  <dialog ref="dialog" class="reference-dialog" aria-labelledby="reference-title" @cancel.prevent="requestClose">
    ...
    <section v-if="state.step === 'source'" aria-labelledby="reference-source-title">
      <h3 id="reference-source-title">1. 登记原资料版本</h3><p>请选择明确提供的结构化 JSON，或粘贴原文。默认不读取书籍、目录或照片。上限 1 MiB / 500 行。</p>
