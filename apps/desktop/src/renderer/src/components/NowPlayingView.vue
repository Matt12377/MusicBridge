        ...
        <div class="now-playing-copy">
          ...
          <div class="now-playing-progress" aria-label="播放进度">
            <div class="now-playing-progress-track" :style="{ '--progress-ratio': `${progressRatio * 100}%` }">
              ...
              <input class="now-playing-progress-input" type="range" aria-label="拖动播放进度" :min="0" :max="Math.max(durationMs, 1)" :value="progressMs" :disabled="!props.currentTrack || durationMs <= 0" @input="previewSeek" @change="commitSeek" />
            </div>
