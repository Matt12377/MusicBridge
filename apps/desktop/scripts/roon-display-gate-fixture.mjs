...
try {
  ...
  for (const socket of sockets) socket.send(frame('Changed', { zones_changed: [{ zone_id: 'zone-one', state: 'playing' }] }))
  await delay(100)
  assert.equal(store.read(context)?.status, 'ready', '仅播放状态变化不能清除当前曲目和歌词')
  for (const socket of sockets) socket.send(frame('LyricsChanged', { zone_id: 'zone-one', lrc: '[00:00.00]状态更新后歌词' }))
  await waitFor(() => store.read(context)?.lines[0]?.text === '状态更新后歌词', '状态更新后丢失当前曲目身份')
  for (const socket of sockets) socket.send(frame('Changed', { zones_changed: [{ zone_id: 'zone-one', now_playing: null }] }))
  await waitFor(() => store.read(context)?.status === 'unavailable', '显式无曲目时未清除旧歌词')
  for (const socket of sockets) socket.send(frame('Changed', { zones_changed: [{ zone_id: 'zone-one', now_playing: {
    three_line: { line1: track.title, line2: track.artist, line3: track.album }, length: 20,
  } }] }))
  for (const socket of sockets) socket.send(frame('LyricsChanged', { zone_id: 'zone-one', lrc: '[00:00.00]恢复后歌词' }))
  await waitFor(() => store.read(context)?.lines[0]?.text === '恢复后歌词', '重新出现曲目后未恢复歌词')
