...
> **主页大封面重排。** `480e37a` 去掉主页外层玻璃气泡；每日推荐、喜欢的音乐和歌单封面采用桌面五列，窄窗三列。每日推荐预览五首，“查看全部”“播放全部”保留完整数据；删除继续聆听板块、Home 当前曲目绑定及无用样式，底部播放器不变。35 项聚焦回归、构建及三档合成原生检查通过，未跑全套测试。

> **侧栏归并两类。** `15d2656` 仅保留“资料库”“收藏”两个主分类，既有十个固定入口完整；网易云歌单作为收藏子项继续支持展开／收起，实物入口显示“实物收藏”。39 项相关回归、构建及合成原生分类／折叠／设置检查通过；未跑完整测试，未改变数据、播放或连接行为。

> **播放栏后方空带修正。** `68591ec` 保留已认可的播放栏样式，改为绝对定位悬浮层，侧栏和右侧面板均延伸至窗口底部；滚动末尾留出避让，侧栏入口仅在可能被覆盖的宽度下上移。34 项 Renderer 回归、构建、diff-check 通过；合成原生 1980/1440/720 三档核对满高、七个控件、设置可点击与滚动末尾可见。未跑完整测试、类型检查或真实播放；本轮只有生产 CSS 变化。

> **居中播放栏与顶部精简。** `9c53aaf` 将播放栏改为居中悬浮玻璃条（最大 1120px），歌曲信息与全部控件保持不变；移除内容顶部横栏和连接状态入口，“未确认操作”移至设置→应用，后台连接不变。按 Owner 速度优先要求，仅跑 44 项聚焦回归、类型、构建和合成原生布局检查；未跑完整单测、完整 E2E 或真实播放。四档宽度 1440/1000/920/720 下七个控件完整、无重叠，设置内未确认操作弹窗可打开。最终观感待反馈。

> **侧栏设置与歌单折叠。** `cab86c5` 移除底部网易云头像／昵称入口，改为全宽毛玻璃“设置”横条；网易云歌单默认展开，可点击标题收起，收窄侧栏仍有歌单弹层。补齐弹层脱离滚动容器、Esc 回焦和窗口边界保护。682/682 桌面单测、类型、生产构建、静态边界门通过；隔离合成 Electron 检查了设置跳转、无头像、展开／折叠以及宽／窄窗口弹层可见性。完整 Playwright、真实播放与 Owner 最终视觉验收不升级。详见 [本轮验证](../design-qa.md)。

> **柔暗与通透度微调。** `549569f` 按 Owner 反馈降低边栏、控件和内容底的浅色遮盖，使用偏灰紫背景 wash，保留封面与静态磨砂纹理，文字同步加深。678/678 桌面单测、生产构建和静态边界门通过；未播放状态已目视检查，播放状态自动截图超时，最终视觉反馈仍待 Owner。未改布局或播放功能。

> **封面背景与磨砂质感返工。** `6f5f4f7` 恢复放大封面：主题不透明度从 10% 调至 82%，降低封面自身模糊以保留轮廓，玻璃面板负责进一步打散细节；增加本地静态颗粒及明暗边缘。背景直接跟随当前曲目，不再因为暂停被移除。677/677 单测、类型、构建、静态边界门通过；合成播放截图已目视检查，暂停状态的原生自动检查未完整通过，不冒充真实播放或 Owner 验收。

> **樱花毛玻璃正式 UI 已实现，等待本轮视觉反馈。** Owner 已认可独立预览并授权迁移；`e3942a3` 将正式应用切换为樱花渐变与透色玻璃。按“边栏及控件不能变成纯色”的反馈，降低结构面板和控件白色遮盖，增加背景模糊和边缘高光。675/675 桌面单测、类型检查、生产构建及静态边界门通过；正式 Electron 合成窗口已打开并聚焦核对主界面材质。这不是全页面视觉验收或真实播放验收，已有真实播放 carryover 不变。见 [本轮设计验证](../design-qa.md)。

> **UI 改版前基线已固化。** 基线分支 `codex/v3-ui-baseline`，完整继承当前开发线 `84f0cf6`；从基线提交创建 `codex/v3-ui-redesign` / `worktree/v3-ui`。本次只固化与隔离，不改变 UI、不合并主干、不发布。真实播放恢复及既有外部门继续保留。见 [UI 基线说明](../reports/V3_UI_BASELINE.md)。

> **当前故障：网易云播放尚未恢复。** 只读运行状态确认 `ROON_TIMEOUT`，远程音频隧道未运行；Roon 控制连接与音频可达性不能混为一谈。IPC 隐藏错误类别的问题已修复，86 项回归、类型检查、桌面构建通过；当前实例未重启，真实播放仍待恢复既有隧道并验证。界面控制返回窗口不可用，需人工完成连接操作。见 [故障记录](../reports/V3_NETEASE_PLAYBACK_TIMEOUT.md)。
...
- [x] 本地曲目上下文队列：`59d2436…` 保留专辑／歌单顺序和起始位置，补齐分页，经有界 IPC 一次建立原生队列，使上一首／下一首可用。189 项聚焦检查、类型与生产构建通过，真实播放仍待验收。
- [x] 磁带参考图功能与授权首批导入：扫描书图片已通过预览与确认发布流程进入本地目录；实物照片优先，参考候选标注来源和版次未核，详情展示多版图片。原来源、旧修订及库存保留。尚未可靠配图的条目不计作完成配图，逐款准确性仍待人工验收。
- [x] 冻结容量执行：旧窗口保持不可重放；objects-limit queued-stop 与 joint 正式链保持 `NOT_RUN`。
- [x] 固化并推送收尾路由 `c848876…`；打包护栏配置修复后最终候选为 `53c1ae8…`，远端分支已对齐。
- [x] 最终 `verify`、静态 Gate、生产构建/打包、DMG/签名校验与包内启动退出 smoke 全部 exit 0；分层证据见收尾结果报告。
- [x] 候选应用已打开，人工验收清单已准备；早期辅助功能树读取超时保留为历史事实，本轮真实辅助树与截图已覆盖 Remote Core 冷启动、seek、队列和专辑分页，Owner 接受仍待实际操作。
...
- [x] Remote Core 验收收口：`1f1d8aa…` 修复网易云 seek、队列 Inspector 布局、队列/Next 提前就绪、远程隧道重启期 `NOT_READY` 竞态与专辑底部自动分页；真实开发版观察到进度 0:36→1:57、队列立即显示 219 首、Next 约 1 秒内可用及专辑 24→48 自动增长。Desktop 657/657、Bridge 聚焦 117/117、类型、构建和静态 Gate 全绿。
- [x] 下一首应用侧准备：`15e7726…` 在当前歌曲播放期间准备紧邻下一首的元数据、短期 URL 与 Range 预检；有界单请求，过期重新解析、失败回退、换队列与停止清理结果。Controller/Roon 122/122，类型检查与生产构建通过。
- [ ] Roon 下一首实际音频延迟：本轮未修改 `track` 模式和会话生命周期，原约 5–6 秒整轨缓冲尚未用真实设备重新测量；应用侧预解析不等于 Roon 缓冲已消除。保留持久会话及 `next` 槽协议核验，不恢复已失败的 channel/chunked 实验。
- [ ] Owner A-15：Remote Core/Zone 已实连并完成资料库、seek、队列/Next 与专辑分页功能观察；仍需 Owner 对声音连续性、切歌体感和整体产品体验作最终接受，自动证据不替代。
