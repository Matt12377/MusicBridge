...
当前进度：TASK-078本地自动软件子范围已在最终HEAD `fac7363b4a6481591e207dda7cca77f0ae8d3cd4` 封版；TASK-079继续在独立分支。当前无设备，只开发fail-closed就绪控制面，不枚举、不打开、不配置设备。objects-limit generation window-03与measure window-06已软件PASS。queued-stop window-01为terminal issuer failure；window-02以历史exact74 authority签发后在admission/child前触发旧replay类型错误，已由提交`ab5f33912e29ec8206358b3c7521d0752710b13b`生成零样本、不可重放的prechild终态收据。下一全新successor闭包为exact75，但仍未签发、formal run仍为`NOT_RUN`，没有预分配UUID/window/label。joint单活动输出预算历史实现`5464ae06355832a76dc394c4cde5eed28acb4846`保持GREEN，正式generation/measure仍为`NOT_RUN`。最新`ab5f339…`已push并fetch核对远端一致；可听Replica、TASK073真实HAL/Gate B、实体纸张与最终Owner仍待验。`main`/PR合并、签名、公证和发布未授权；智能体统一GPT-5.6 Sol / High。

本表是任务拆分与依赖计划，不是完成声明。后续任务沿上一任务最终 HEAD 创建独立分支；当前已展开 TASK-064～078，其余任务开始前补详细范围和允许路径。具体子任务可根据已验证结果细分，不删减 PRD 范围。

## 实时进度面板

> **当前执行：** `TASK-079 / window-02 prechild终态已封存，正在同步机器状态并准备successor exact75 fresh只读预检`。实现=`ab5f33912e29ec8206358b3c7521d0752710b13b`已push并fetch核对；window-02收据SHA=`0b372f0c…`，无admission、supervision、child、benchmark、output或样本，永久禁止重放。下一successor=`exact 75`、authority=`NEXT_NOT_ISSUED`、formal run=`NOT_RUN`，没有UUID/window/label；joint实现`5464ae…`仅为软件预算GREEN，正式generation/measure、设备Gate B和Owner验收均为`NOT_RUN`。
...
> **queued-stop first-class控制面：** window-01首次签发尝试的基础闭包为71个冻结measure roots + authority parent + issuer identity=`exact 73`，但只留下terminal issuer failure，不是approved authority或运行收据。window-02把该failure逐项带入issuer fact、authority、admission与terminal身份快照，历史闭包为`exact 74`；它已在child前终止并永久禁用。下一successor还必须带入window-02 prechild failure，闭包为`exact 75`。输出仍位于authority parent内；issuer只签发、不执行benchmark，supervisor精确校验843行aggregate与成功终态636个输出文件。successor须绑定同步状态后的精确远端HEAD重新预检、使用全新名称签发且只消费一次。
...
- [ ] TASK-079 / objects-limit queued-stop正式窗口：window-01与window-02均已terminal且永久禁止重放；下一全新successor仍未签发、formal benchmark=`NOT_RUN`。状态检查点push后必须绑定新的精确远端HEAD做fresh只读预检，只能以全新UUID/window-dir/label签发，不得预先分配身份或写成PASS。
...
- [x] TASK-079 / 外部评审分支检查点：验证清洁的`codex/task-079-v3-final-acceptance`实现检查点`ab5f33912e29ec8206358b3c7521d0752710b13b`已push并fetch确认远端同SHA；`5464ae…`保留为joint历史实现锚。只开放分支评审，`main`/PR合并、签名、公证、安装和发布仍未授权。
