...
> **樱花玻璃独立预览待审美反馈。** `codex/v3-ui-redesign` 新增 `prototypes/sakura-glass/index.html`；只有合成曲目和示意素材。搜索、模拟切歌、进度及队列可交互；正式应用源码、真实服务和用户数据未改动。先确认视觉方向，再决定正式 UI 迁移；不将预览交互视为真实播放验收。
