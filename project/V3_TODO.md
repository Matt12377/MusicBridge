...
当前进度：TASK-078本地自动软件子范围已在最终HEAD `fac7363b4a6481591e207dda7cca77f0ae8d3cd4` 封版；TASK-079继续在独立分支。当前无设备，只开发fail-closed就绪控制面，不枚举、不打开、不配置设备。objects-limit generation window-03与measure window-06已软件PASS。queued-stop window-01的terminal issuer failure已进入下一authority的exact74闭包，但下一approved authority仍未签发、formal run仍为`NOT_RUN`，也没有预分配后继UUID/window/label。joint单活动输出预算软件检查点`5464ae06355832a76dc394c4cde5eed28acb4846`已GREEN并推送，远端SHA已精确复核；planned bytes从`6,140,461,056`收敛为`2,701,131,776`，正式generation/measure仍为`NOT_RUN`。可听Replica、TASK073真实HAL/Gate B、实体纸张与最终Owner仍待验。`main`/PR合并、签名、公证和发布未授权；智能体统一GPT-5.6 Sol / High。

本表是任务拆分与依赖计划，不是完成声明。后续任务沿上一任务最终 HEAD 创建独立分支；当前已展开 TASK-064～078，其余任务开始前补详细范围和允许路径。具体子任务可根据已验证结果细分，不删减 PRD 范围。

## 实时进度面板

> **当前执行：** `TASK-079 / joint单活动输出预算软件GREEN，同步机器状态后对最新远端HEAD重做window-02只读预检`。joint实现=`5464ae06355832a76dc394c4cde5eed28acb4846`并已推送精确远端SHA；预算模型=`serial-single-output-plus-bounded-growth-v1`，`activeOutputMaximum=1`，planned=`2,701,131,776`字节；capacity=`92/92`、supervisor=`32/32`、readiness=`15/15`，独立终审P0/P1/P2均为0。queued-stop下一闭包仍为`exact 74`，approved authority=`NEXT_NOT_ISSUED`、formal run=`NOT_RUN`，没有后继UUID/window/label；joint正式generation/measure、设备Gate B和Owner验收均为`NOT_RUN`。
...
> **queued-stop first-class控制面：** window-01首次签发时闭包为71个冻结measure roots + authority parent + issuer identity=`exact 73`。该次只留下一个terminal issuer failure，不是approved authority或运行收据；从下一窗口开始，这个direct-child failure必须逐项进入issuer fact、authority、admission和terminal身份快照，因此闭包动态增至`exact 74`。输出仍位于authority parent内。issuer只签发、不执行benchmark；supervisor精确校验843行aggregate与成功终态636个输出文件。下一窗口仍须绑定同步状态后的精确远端HEAD重新预检、使用全新名称签发且只消费一次。
...
- [x] TASK-079 / queued-stop prior issuer failure carryover：提交`f285bf3…`要求声明失败集合与runtime direct-child queued failure精确相等，并把window-01 failure、issuer fact、owner、installed supervisor的目录和文件身份带入issue/admission/terminal；下一闭包动态为exact74 roots。capacity 92/92、supervisor 28/28、issuer 9/9、四套控制面81/81及全量软件Gate GREEN；该代码检查点已推送并精确复核远端SHA。
- [ ] TASK-079 / objects-limit queued-stop正式窗口：window-01在issuer阶段以`SOURCE_CANDIDATE`终止，UUID=`c9e11b19…`、failure SHA=`e18619e0…`、approved window未写、benchmark未运行且永久禁止重放。当前同步TODO/STATUS/READINESS后做精确远端HEAD新鲜只读预检；下一authority仍未签发、formal run=`NOT_RUN`，不得预先分配后继UUID或写成PASS。
- [x] TASK-079 / joint软件容量预算：提交`5464ae06…`将Plan改为逐Record串行创建/消费，封存`preparedBeforeFirstAttempt=1`、`activePlanMaximum=1`、`unconsumedAtSeal=1`；写snapshot前拒绝超出冻结计划的投影，generation artifacts与measure seed共用严格消费合同。这只关闭软件预算重构，正式generation/measure仍未运行。
- [ ] TASK-079 / joint正式generation/measure：必须先通过当前HEAD新鲜空间/身份预检，使用唯一签发的authority与全新名称；不得以软件GREEN升级为joint正式PASS、整个TASK-079、设备Gate B或Owner验收。
- [x] TASK-079 / 外部评审分支检查点：验证清洁的`codex/task-079-v3-final-acceptance`实现检查点`5464ae06355832a76dc394c4cde5eed28acb4846`已push；`git ls-remote`确认远端同SHA。只开放分支评审，`main`/PR合并、签名、公证、安装和发布仍未授权。
