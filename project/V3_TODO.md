...
当前进度：TASK-078本地自动软件子范围已在最终HEAD `fac7363b4a6481591e207dda7cca77f0ae8d3cd4` 封版；TASK-079继续在独立分支。当前无设备，只开发fail-closed就绪控制面，不枚举、不打开、不配置设备。objects-limit generation window-03与measure window-06已软件PASS。queued-stop window-06以`PROCESS_EXIT`形成零样本终态；window-07已唯一签发并只消费一次，但installed supervisor在authority admission前以`CAPACITY_SUPERVISOR_INPUT`拒绝，未启动supervision/child/benchmark、未创建output或样本。三位置停止后，架构检查点`ed73b5984a30f2f7a3d9d1505969795dfa5d68cb`以版本化JSON合同、共享Python纯函数、TypeScript等价解释器和同一golden corpus统一direct head、递归前驱深度、计费根、时序、PID与身份语义；issuer、installed supervisor、TypeScript consumer均已GREEN。因此原“三位置语义分叉”架构阻塞已解除，但没有签发、授权或运行新窗口；正式queued-stop仍为`NOT_RUN`。joint正式generation/measure、可听Replica、TASK073真实HAL/Gate B、实体纸张与最终Owner仍为`NOT_RUN`或待验。`main`/PR合并、签名、公证和发布未授权；智能体统一GPT-5.6 Sol / High。

本表是任务拆分与依赖计划，不是完成声明。后续任务沿上一任务最终 HEAD 创建独立分支；当前已展开 TASK-064～078，其余任务开始前补详细范围和允许路径。具体子任务可根据已验证结果细分，不删减 PRD 范围。

## 实时进度面板

> **当前执行：** `TASK-079 / queued-stop统一谱系合同GREEN，等待新鲜审计与显式新窗口授权`。window-06 close SHA=`0f48aa49…`、零样本、terminal/nonreplay；window-07 UUID=`5f5df917…`、window SHA=`0bac4fcc…`已唯一消费并在authority admission前拒绝，无supervision、child、benchmark、output或样本，不重放。架构实现=`ed73b5984a30f2f7a3d9d1505969795dfa5d68cb`；共享corpus覆盖leaf、depth1/2/3及direct/orphan/cycle/fork/time/PID/identity/depth失败。正式queued-stop、joint generation/measure、设备Gate B和Owner验收均未升级。
...
- [x] TASK-079 / queued-stop PROCESS_EXIT统一谱系合同：实现`ed73b5984a30f2f7a3d9d1505969795dfa5d68cb`增加版本化JSON合同、共享Python evaluator、TS evaluator与单一golden corpus；issuer、installed supervisor、TS consumer同时消费direct/recursive/billing/time/PID/identity语义。issuer 71/71、supervisor 58/58、Bridge capacity+conformance与typecheck、标准verify、control/boundaries/cycles/readiness/diff-check全部GREEN。原三位置架构语义阻塞解除。
...
- [x] TASK-079 / 架构评审分支检查点：实现`ed73b5984a30f2f7a3d9d1505969795dfa5d68cb`、状态报告`e9416cb7e2510da327598798bff6f448be19c8a9`已push；HEAD/upstream/FETCH_HEAD/ls-remote四方核对同SHA。`5464ae…`保留为joint历史实现锚。只开放分支评审，`main`/PR合并、签名、公证、安装和发布仍未授权。
