...
| R-023 | TASK074 每次Attempt写入同步重演全部历史/回执，累计进度可能产生二次增长的校验工作量并拖慢停止响应 | 当前生产Runtime不注入provider，正式Begin始终拒绝；预算有界但未据此宣称实时响应；本地行为Gate不替代容量测量 | TASK078测量最大支持规模下的进度写入、停止与冷启延迟，优化增量校验且保留完整恢复审计；真实driver接入前必须解除，不以ACK替代实际停止 |
