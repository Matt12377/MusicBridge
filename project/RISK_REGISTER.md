...
| R-021 | TASK073实际应用包正常Quit在纠正探针后仍15秒超时，仅Main残留 | 失败与SIGKILL清理证据保留，已确认无残留；不修改Fuses/sender或继续盲跑；无设备检查和UI通过不覆盖退出FAIL | 独立取得退出阶段/主线程根因证据，修复后实际包正常child close为code0且无强制信号；关闭前不宣称正常退出或发布验收通过 |
