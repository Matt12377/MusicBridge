...
| ID | 风险 | 当前控制 | 关闭条件 |
|---|---|---|---|
| R-020 | TASK-066 普通冷启2秒ready与完整数据库/候选扫描可能在大库或慢盘超时 | 当前合成Gate通过；只做有界失败或回滚，不覆盖旧库，不宣称容量通过 | TASK-078/Gate E以实际规模合成库测量并统一冷启超时策略；发布前关闭 |
| R-021 | TASK073实际应用包正常Quit在纠正探针后仍15秒超时，仅Main残留 | 旧失败与SIGKILL清理证据保留；第四阶段新包无CDP启动自退出code0/close约703ms、无残留，但未复测执行后退出；不放松Fuses/sender或盲跑，旧场景FAIL不被启动通过覆盖 | 独立取得退出阶段/主线程根因证据，修复后实际包正常child close为code0且无强制信号；关闭前不宣称正常退出或发布验收通过 |
...
| R-023 | TASK074 每次Attempt写入同步重演全部历史/回执；TASK075又加入Record/当前内容/照片闭包校验及逐条冻结Plan搜索，TASK077 schema21再加入打印对象闭包核验，累计工作量可能拖慢停止、检索和冷启 | 当前生产Runtime不注入provider，正式Begin始终拒绝；预算有界但未据此宣称实时响应；本地行为Gate不替代容量测量 | TASK078测量最大支持规模下的进度写入、停止、档案检索、照片字节校验与冷启延迟，优化增量校验且保留完整恢复审计；真实driver接入前必须解除，不以ACK替代实际停止 |
