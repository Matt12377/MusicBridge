...
test('carryover 嵌套 primitive 使用稳定终止或覆盖错误而非内部异常', () => {
  const malformedTerminalWindow = fixture()
  json(join(malformedTerminalWindow.runtime, 'old-window/window.json'), 'terminal-window')
  const terminalWindowClose = JSON.parse(readFileSync(malformedTerminalWindow.close, 'utf8'))
  terminalWindowClose.window.sha256 = sha(join(malformedTerminalWindow.runtime, 'old-window/window.json'))
  json(malformedTerminalWindow.close, terminalWindowClose)
  const terminalWindowInventory = JSON.parse(readFileSync(malformedTerminalWindow.inventory, 'utf8'))
  terminalWindowInventory.terminalClose.sha256 = sha(malformedTerminalWindow.close)
  json(malformedTerminalWindow.inventory, terminalWindowInventory)
  const terminalWindowResult = spawnSync(python, args(malformedTerminalWindow), { encoding: 'utf8' })
  assert.notEqual(terminalWindowResult.status, 0)
  assert.match(terminalWindowResult.stderr, /REPLAY_AUDIT/)
  assert.doesNotMatch(terminalWindowResult.stderr, /ISSUER_INTERNAL/)

  const malformedSafety = fixture()
  const safetyClose = JSON.parse(readFileSync(malformedSafety.close, 'utf8'))
  safetyClose.safety = 'unsafe'
  json(malformedSafety.close, safetyClose)
  const safetyInventory = JSON.parse(readFileSync(malformedSafety.inventory, 'utf8'))
  safetyInventory.terminalClose.sha256 = sha(malformedSafety.close)
  json(malformedSafety.inventory, safetyInventory)
  const safetyResult = spawnSync(python, args(malformedSafety), { encoding: 'utf8' })
  assert.notEqual(safetyResult.status, 0)
  assert.match(safetyResult.stderr, /CARRYOVER_TERMINAL/)
  assert.doesNotMatch(safetyResult.stderr, /ISSUER_INTERNAL/)

  const malformedCoverage = fixture()
  const coverageClose = JSON.parse(readFileSync(malformedCoverage.close, 'utf8'))
  coverageClose.partialEvidence = 'partial-output'
  json(malformedCoverage.close, coverageClose)
  const coverageInventory = JSON.parse(readFileSync(malformedCoverage.inventory, 'utf8'))
  coverageInventory.terminalClose.sha256 = sha(malformedCoverage.close)
  json(malformedCoverage.inventory, coverageInventory)
  const coverageResult = spawnSync(python, args(malformedCoverage), { encoding: 'utf8' })
  assert.notEqual(coverageResult.status, 0)
  assert.match(coverageResult.stderr, /CARRYOVER_COVERAGE/)
  assert.doesNotMatch(coverageResult.stderr, /ISSUER_INTERNAL/)

  const malformedSupervisor = fixture()
  const terminalSupervisorPath = join(malformedSupervisor.runtime, 'old-window/supervision/supervisor.json')
  json(terminalSupervisorPath, { generation: 'invalid' })
  const supervisorClose = JSON.parse(readFileSync(malformedSupervisor.close, 'utf8'))
  supervisorClose.supervisor.sha256 = sha(terminalSupervisorPath)
  json(malformedSupervisor.close, supervisorClose)
  const supervisorInventory = JSON.parse(readFileSync(malformedSupervisor.inventory, 'utf8'))
  supervisorInventory.terminalClose.sha256 = sha(malformedSupervisor.close)
  json(malformedSupervisor.inventory, supervisorInventory)
  const supervisorResult = spawnSync(python, args(malformedSupervisor), { encoding: 'utf8' })
  assert.notEqual(supervisorResult.status, 0)
  assert.match(supervisorResult.stderr, /CARRYOVER_COVERAGE/)
  assert.doesNotMatch(supervisorResult.stderr, /ISSUER_INTERNAL/)
})
