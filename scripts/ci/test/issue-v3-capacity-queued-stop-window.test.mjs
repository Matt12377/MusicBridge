...
function candidate(root) {
  ...
  cpSync(sourceLineageHelper,join(root,'scripts/ci/capacity_process_failure_lineage.py'))
  cpSync(sourceLineageContract,join(root,'packages/contracts/capacity-process-failure-lineage-v1.json'))
...
function linkedProcessFailure(f, mutate = () => {}) {
  ...
  const close=JSON.parse(readFileSync(head.close));close.closedAt='2026-08-30T10:52:58.666686+00:00'
  close.windowSha256=sha(head.window);close.ownedManifestSha256=sha(head.owned)
