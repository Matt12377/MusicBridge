...
function priorProcessFailure(f, suffix = '') {
  ...
  return {parent,windowId,windowDirName,label,close,historicalRoots,stderr,supervision,
...
test('PROCESS_EXIT exact75非自身根逐项绑定71 measure加issuer/prechild顺序',async t=>{for(const [name,mutate] of [['新增任意根',roots=>roots.push(structuredClone(roots[0]))],['marker替换',roots=>{roots[0].marker.sha256='f'.repeat(64)}],['issuer-prechild互换',roots=>{[roots[71],roots[72]]=[roots[72],roots[71]]}]])await t.test(name,()=>{const f=fixture();try{const prior=priorProcessFailure(f),expected=structuredClone(prior.historicalRoots);mutate(expected);const o=options(f);o.prior_process_failure=[prior.argv];const r=pythonCall("o=types.SimpleNamespace(**json.loads(sys.argv[2]));\ntry:m.validate_prior_process_failures(o,m.Path(sys.argv[3]),json.loads(sys.argv[4]))\nexcept m.IssueError as e:print(e);raise SystemExit(1)",JSON.stringify(o),f.runtime,JSON.stringify(expected));assert.equal(r.status,1,r.stderr);assert.match(r.stdout,/PRIOR_PROCESS_FAILURE/u)}finally{cleanup(f)}})})
...
test('PROCESS_EXIT验证在UUID前执行、绑定facts并在发布前二次比对快照',()=>{const source=readFileSync(sourceIssuer,'utf8'),issue=source.slice(source.indexOf('def issue(options):'));const first=issue.indexOf('process_failures = validate_prior_process_failures('),uuid=issue.indexOf('window_id = str(uuid.uuid4())'),pending=issue.indexOf("pending = parent / 'window.pending.json'"),second=issue.indexOf('second_process_failures = validate_prior_process_failures('),publish=issue.indexOf("os.rename(pending, parent / 'window.json')");assert.ok(first>=0&&first<uuid);assert.ok(second>pending&&second<publish);assert.match(issue,/expected_process_inherited = \[\*measure\['roots'\], \*prior_failures\['roots'\],/u);assert.match(issue,/second_expected_process_inherited = \[\*second_measure\['roots'\], \*second_prior_failures\['roots'\],/u);assert.match(issue,/'processFailureCarryover': process_failures\['facts'\]/u);assert.match(issue,/second_process_failures\['snapshots'\] != process_failures\['snapshots'\]/u);const roots=pythonCall("print(json.dumps([m.EXPECTED_PREFLIGHT_ROOTS,m.EXPECTED_AUTHORITY_ROOTS]))");assert.equal(roots.status,0,roots.stderr);assert.deepEqual(JSON.parse(roots.stdout),[74,76])})
