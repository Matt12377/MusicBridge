#!/usr/bin/env python3
"""签发 joint measure 一次性 authority；只消费 joint:generate:PASS，不执行 benchmark。"""
...
def _strict_json(path, expected_sha, maximum=32 * 1024 * 1024):
    try:
        ...
        raise IssueError('JOINT_GENERATION_PASS') from error


def validate_joint_generation_pass(value):
    """验证joint generation完整终态；不把exit 0或supervisor code 0单独当PASS。"""
    try:
        window = value['window']
        close = value['close']
        ...
        window_valid = _mapping(window) \
            ...
            and window.get('scope') == 'musicbridge-capacity-generation-window' \
            and window.get('phase') == 'generate' \
            and window.get('profile') == 'joint' \
            and window.get('state') == 'approved' \
            and SAFE.fullmatch(str(window.get('label', ''))) is not None \
            and type(window.get('n')) is int and window['n'] == 1
        close_valid = _mapping(close) \
            and close.get('scope') == 'musicbridge-capacity-generation-window-close' \
            and close.get('windowId') == window.get('id') \
            and close.get('profile') == 'joint' \
            ...
            and close.get('generation') == generation
        supervision_valid = _mapping(supervision) \
            ...
            and _mapping(generation) and generation.get('verifiedPassed') is True \
            and generation.get('exitZero') is True \
            and generation.get('sourceBeforeEqualsAfter') is True \
            and generation.get('fixtureIdentityValid') is True \
            and generation.get('authorityStable') is True \
            and generation.get('jointSeedValid') is True \
            and generation.get('generationPlanValid') is True \
            and generation.get('generationSpaceValid') is True \
            and generation.get('noSqliteSidecars') is True
        files_valid = _mapping(files) and set(files) == {
            ...
            'seedSnapshotSha256', 'fixtureOwnerSha256',
        ...
        raise IssueError('JOINT_GENERATION_PASS') from error
    return {
        'requiredResult': 'joint:generate:PASS',
        'windowId': window['id'],
        'label': window['label'],
        **files,
    }
...
def load_joint_generation_pass(*, window_path, window_sha, close_path, close_sha,
                               supervision_path, supervision_sha, owned_path, owned_sha,
                               source_path, source_sha, expected_candidate,
                               expected_supervisor_sha):
    """从joint generation原始证据重建前驱PASS，并闭合seed/fixture身份。"""
    try:
        ...
        _, installed_supervisor_sha = _stable_sha256(
            installed_supervisor, 'JOINT_GENERATION_PASS')
        issuer_fact_path = parent / 'issuer-identity' / 'owner.json'
        _, issuer_fact_sha = _stable_sha256(issuer_fact_path, 'JOINT_GENERATION_PASS')
        issuer_fact = _strict_json(issuer_fact_path, issuer_fact_sha)
        generation_issuer = Path(expected_candidate['root']) \
            / 'scripts/ci/issue-v3-capacity-joint-generation-window.py'
        _, generation_issuer_sha = _stable_sha256(
            generation_issuer, 'JOINT_GENERATION_PASS')
        if window.get('schemaVersion') != 1 \
                or window.get('scope') != 'musicbridge-capacity-generation-window' \
                ...
                or close.get('scope') != 'musicbridge-capacity-generation-window-close' \
                ...
                or installed_supervisor_sha != expected_supervisor_sha \
                or issuer_fact.get('schemaVersion') != 1 \
                or issuer_fact.get('scope') != 'musicbridge-capacity-joint-generation-authority-issuer' \
                or issuer_fact.get('windowId') != window.get('id') \
                or issuer_fact.get('candidateRepository') != expected_candidate \
                or issuer_fact.get('supervisor') != {
                    'path': str(installed_supervisor), 'sha256': expected_supervisor_sha} \
                or issuer_fact.get('issuer') != {
                    'path': str(generation_issuer), 'sha256': generation_issuer_sha} \
                or issuer_fact.get('authorityInherited') is not False \
                or issuer_fact.get('receiptReuseAllowed') is not False \
                or issuer_fact.get('oldWindowReplayAllowed') is not False:
            fail('JOINT_GENERATION_PASS')
        output = Path(supervision.get('generation', {}).get('outputDirectory', ''))
        if not output.is_absolute() or output != Path(window_path).parent.parent / window.get('label', ''):
            fail('JOINT_GENERATION_PASS')
        metadata_path = output / 'seed.json'
        snapshot_path = output / 'seed.sqlite'
        metadata_sha = supervision.get('generation', {}).get('seedMetadataSha256Observed')
        snapshot_sha = supervision.get('generation', {}).get('snapshotSha256Observed')
        metadata = _strict_json(metadata_path, metadata_sha)
        _, observed_snapshot_sha = _stable_sha256(snapshot_path, 'JOINT_GENERATION_PASS')
        fixture_path = Path(str(metadata.get('fixtureDirectory', '')))
        fixture_sha = supervision.get('generation', {}).get('fixtureIdentity', {}).get('markerSha256')
        _, observed_fixture_sha = _stable_sha256(
            fixture_path / 'capacity-owner.json', 'JOINT_GENERATION_PASS')
        if observed_snapshot_sha != snapshot_sha or observed_fixture_sha != fixture_sha \
                or metadata.get('profile') != 'joint' \
                or metadata.get('snapshotSha256') != snapshot_sha:
            fail('JOINT_GENERATION_PASS')
        return validate_joint_generation_pass({
            'window': window,
            'close': close,
            ...
            'files': {
                ...
                'seedMetadataSha256': metadata_sha,
                'seedSnapshotSha256': snapshot_sha,
                'fixtureOwnerSha256': fixture_sha,
            },
        })
    except IssueError:
        raise
...
def build_authority_payload(*, predecessor, window_id, label, issued_at, deadline_at,
    ...
    predecessor_fact = validate_joint_generation_pass(predecessor)
    issued = _canonical_time(issued_at)
    deadline = _canonical_time(deadline_at)
    if deadline - issued != datetime.timedelta(seconds=900) \
    ...
    window = {
        ...
        'profile': 'joint',
        'label': label,
        'seedLabel': predecessor_fact['label'],
        'n': 105,
        'issuedAt': issued_at,
    ...
    fact = {
        'schemaVersion': 1,
        'scope': 'musicbridge-capacity-joint-measure-authority-issuer',
        'windowId': window_id,
        'candidateRepository': dict(candidate),
...
def _candidate(root, expected_branch, expected_head, issuer, expected_issuer_sha,
    ...
    if _git(root, 'rev-parse', '--show-toplevel') != str(root) \
        ...
        fail('REPOSITORY_IDENTITY')
    expected_issuer = root / 'scripts/ci/issue-v3-capacity-joint-measure-window.py'
...
def _parse_args(argv):
    ...
    parser.add_argument('--joint-generation-window', required=True)
    parser.add_argument('--expected-joint-generation-window-sha256', required=True)
    parser.add_argument('--joint-generation-close', required=True)
    parser.add_argument('--expected-joint-generation-close-sha256', required=True)
    parser.add_argument('--joint-generation-supervision', required=True)
    parser.add_argument('--expected-joint-generation-supervision-sha256', required=True)
    parser.add_argument('--joint-generation-owned-manifest', required=True)
    parser.add_argument('--expected-joint-generation-owned-sha256', required=True)
    parser.add_argument('--joint-generation-source-manifest', required=True)
    parser.add_argument('--expected-joint-generation-source-sha256', required=True)
...
def _record_failure(code):
    ...
    try:
        _exclusive_json(context['parent'] / 'issuer-failure.json', {
            'schemaVersion': 1,
            'scope': 'musicbridge-capacity-joint-measure-authority-issuer-failure',
...
def issue(options):
    ...
    predecessor = load_joint_generation_pass(
        window_path=options.joint_generation_window,
        window_sha=options.expected_joint_generation_window_sha256,
        close_path=options.joint_generation_close,
        close_sha=options.expected_joint_generation_close_sha256,
        supervision_path=options.joint_generation_supervision,
        supervision_sha=options.expected_joint_generation_supervision_sha256,
        owned_path=options.joint_generation_owned_manifest,
        owned_sha=options.expected_joint_generation_owned_sha256,
        source_path=options.joint_generation_source_manifest,
        source_sha=options.expected_joint_generation_source_sha256,
        expected_candidate=candidate,
        expected_supervisor_sha=options.expected_supervisor_sha256)
    predecessor_parent = Path(options.joint_generation_window).parent
    if predecessor_parent.parent != runtime:
        fail('JOINT_GENERATION_PASS')
    predecessor_owned = _strict_json(
        options.joint_generation_owned_manifest, options.expected_joint_generation_owned_sha256)
    roots = [_owned_root(row) for row in predecessor_owned['roots']]
    generation_supervision = _strict_json(
        options.joint_generation_supervision,
        options.expected_joint_generation_supervision_sha256)
    seed_directory = Path(generation_supervision['generation']['outputDirectory'])
    try:
        verification_contract = runpy.run_path(
            str(supervisor), run_name='musicbridge_joint_generation_evidence_replay')
        generation_window = _strict_json(
            options.joint_generation_window,
            options.expected_joint_generation_window_sha256)
        expected_generation = {
            'profile': 'joint', 'label': generation_window['label'],
            'window': generation_window['id'],
            'node': '/Users/yihe/.nvm/versions/node/v22.23.2/bin/node',
            'entry': root / 'packages/bridge-core/test/benchmarks/recording-capacity.ts',
            'root': root,
            'windowSha256': options.expected_joint_generation_window_sha256,
            'ownedManifestSha256': options.expected_joint_generation_owned_sha256,
            'sourceManifestSha256': options.expected_joint_generation_source_sha256,
            'authorityProbe': lambda: {
                'authorityStable': True, 'sourcePinsValid': True,
                'ownedRootsValid': True, 'spaceValid': True},
        }
        replayed_generation = verification_contract['_generation_artifacts'](
            runtime, generation_window['label'], expected_generation)
    except IssueError:
        raise
    except Exception as error:
        raise IssueError('JOINT_GENERATION_PASS') from error
    if replayed_generation != generation_supervision.get('generation') \
            or replayed_generation.get('verifiedPassed') is not True:
        fail('JOINT_GENERATION_PASS')
    seed_metadata = _strict_json(seed_directory / 'seed.json', predecessor['seedMetadataSha256'])
    fixture_directory = Path(seed_metadata['fixtureDirectory'])
    roots.extend((
        _current_root(seed_directory, 'seed.json', predecessor['seedMetadataSha256']),
        _current_root(fixture_directory, 'capacity-owner.json', predecessor['fixtureOwnerSha256']),
    ))
    ...
    owner_sha = _exclusive_json(parent / 'owner.json', {
        'scope': 'musicbridge-capacity-measure-window', 'owner': 'root', 'id': window_id})
    ...
    deadline_at = (issued + datetime.timedelta(seconds=900)).isoformat(timespec='milliseconds')
    placeholder = '0' * 64
    predecessor_value = {
        'window': _strict_json(options.joint_generation_window,
                               options.expected_joint_generation_window_sha256),
        'close': _strict_json(options.joint_generation_close,
                              options.expected_joint_generation_close_sha256),
        'supervision': generation_supervision,
        'files': {
            'windowSha256': options.expected_joint_generation_window_sha256,
            'closeSha256': options.expected_joint_generation_close_sha256,
            'supervisionSha256': options.expected_joint_generation_supervision_sha256,
            'ownedManifestSha256': options.expected_joint_generation_owned_sha256,
            'sourceManifestSha256': options.expected_joint_generation_source_sha256,
            'seedMetadataSha256': predecessor['seedMetadataSha256'],
            'seedSnapshotSha256': predecessor['seedSnapshotSha256'],
            'fixtureOwnerSha256': predecessor['fixtureOwnerSha256'],
        },
    }
    first = build_authority_payload(
        predecessor=predecessor_value, window_id=window_id, label=options.label, issued_at=issued_at,
    ...
    owned = {'schemaVersion': 1, 'scope': 'musicbridge-capacity-owned-roots',
             ...
             'futureRoots': [str(runtime / options.label)]}
    ...
    try:
        ...
        if supervisor_contract.get('_MEASURE_LIMITS') != MEASURE_LIMITS \
                or supervisor_contract.get('_MEASURE_PLAN') != MEASURE_PLAN:
            fail('SUPERVISOR_CONTRACT')
        source_result = supervisor_contract['_validate_source_manifest'](
            parent / 'source-pins.json', root)
        planned_bytes = supervisor_contract['_measure_planned_bytes'](
            (seed_directory / 'seed.sqlite').stat().st_size)
        owned_result = supervisor_contract['_validate_owned_manifest'](
            parent / 'owned-roots.json', runtime, window_id, 'joint',
            planned_bytes=planned_bytes, future_path=runtime / options.label,
            future_state='absent')
    except IssueError:
        raise
    except Exception as error:
    ...
    if source_result.get('fileCount') != options.expected_source_count \
            or owned_result.get('rootCount') != len(owned['roots']) + 1 \
            or owned_result.get('plannedBytes') != planned_bytes:
    ...
    if final['issuerFact'] != first['issuerFact']:
        fail('AUTHORITY_PAYLOAD')
    try:
        supervisor_contract['_validate_measure_window'](
            final['window'], issued.timestamp() + 0.001)
...
def main(argv):
    try:
        ...
        print(f'JOINT_MEASURE_ISSUER_{error}', file=sys.stderr)
        return 1
    except Exception:
        _record_failure('INTERNAL')
        print('JOINT_MEASURE_ISSUER_INTERNAL', file=sys.stderr)
        return 1
