...
function preserved(db: DatabaseSync) {
  return db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT GLOB 'recording_plan*' AND name NOT GLOB 'recording_attempt*' AND name NOT GLOB 'recording_record*' AND name NOT GLOB 'sqlite_*' AND name NOT GLOB 'spreadsheet_*' AND name NOT GLOB 'collection_progress_*' AND name NOT GLOB 'collection_want*' ORDER BY name").all().map(({ name }) => [name, db.prepare(name === 'inventory_lots' ? 'SELECT id,sku_id,acquired,sealed,opened,legacy,unknown FROM inventory_lots ORDER BY rowid' : `SELECT * FROM ${name} ORDER BY rowid`).all()]);
}
test('固定schema15迁移20保留原库存/实体/照片/目录快照，新增调整额为0且外键恢复开启', async t => {
  ...
  try {
    assert.equal(db.prepare('PRAGMA user_version').get()?.user_version, 20); assert.deepEqual(preserved(db), history);
