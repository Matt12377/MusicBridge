...
test('固定旧schema14迁移为当前schema18，库存/照片/永久编号和原账本逐行守恒', async t => {
  ...
  try {
    assert.equal(after.prepare('PRAGMA user_version').get()?.user_version, 18);
...
test('schema14目录迁移提交前中断回滚整个迁移，冷开可再次迁移且不改旧库存', async t => {
  ...
  const after = new DatabaseSync(filePath, { readOnly: true });
  try { assert.equal(after.prepare('PRAGMA user_version').get()?.user_version, 18); } finally { after.close(); }
});
