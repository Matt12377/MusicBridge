...
test('固定旧schema14仍可校验，正式schema20迁移只增加零调整额且隔离恢复保留原逐列事实', async t => {
  ...
  assert.equal(inspect().version, 20, 'schema20必须由正式迁移创建，不能只改user_version');
  assert.deepEqual(readBackupIndex(filePath).index, { operations: [], objects: [], incompleteOperationIds: [] });
  isolateRestoredDatabase(filePath); verifyRestoredDatabaseIsolation(filePath);
  assert.deepEqual(inspect(), { ...original, version: 20, lots: original.lots.map(lot => ({ ...lot, quantity_adjustment: 0 })) });
});
...
test('正式schema20隔离副本撤销路径授权但逐列保留固定旧schema15库存与目录事实', async t => {
...
test('schema20工作簿源bytes篡改即使恢复原不可变trigger也拒绝备份校验，原库不受损', async t => {
...
test('固定schema16备份只读验证原bytes，迁移20后完整性及隔离逐列保留导入事实', async t => {
...
test('schema20求购版本篡改恢复trigger后仍被备份full check拒绝，校验不写损坏副本', async t => {
  ...
  const result = await repository.backupSnapshot({ ...await authorizeSourceDirectory(directory), id: randomUUID() }); assert.equal(result.schemaVersion, 20);
...
test('固定schema17备份只读验证，升级20及隔离恢复不修改旧列事实', async t => {
  ...
  const db = new DatabaseSync(filePath, { readOnly: true });
  try { assert.equal(db.prepare('PRAGMA user_version').get()?.user_version, 20); } finally { db.close(); }
