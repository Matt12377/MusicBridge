...
test('schema 11 到 12 回滚保留既有 Profile 版本、Session 和幂等结果', async t => {
  ...
  f.repository.close(); const db = new DatabaseSync(f.filePath); db.exec('DROP TABLE archive_references; DROP TABLE archive_objects; DROP TABLE archive_operations; DROP TABLE archive_roots; DROP TABLE execution_assets; DROP TABLE execution_jobs; DROP TABLE execution_ledger; PRAGMA user_version=11'); db.close();
