...
test('Prepared schema 9 升级故障完整回滚，原工作区与母版历史不变', async t => {
  ...
  const db = new DatabaseSync(f.filePath); try { db.exec('DROP TABLE execution_assets; DROP TABLE execution_jobs; DROP TABLE execution_ledger; DROP TABLE recording_sessions; DROP TABLE recording_profile_versions; DROP TABLE recording_profiles; DROP TABLE recording_profile_ledger; DROP TABLE prepared_versions; DROP TABLE prepared_jobs; DROP TABLE prepared_selections; DROP TABLE prepared_ledger; PRAGMA user_version=9'); } finally { db.close(); }
