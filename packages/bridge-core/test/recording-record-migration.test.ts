...
function facts(db: DatabaseSync) {
  return db.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT GLOB 'sqlite_*' AND name NOT GLOB 'recording_record*' ORDER BY name").all()
...
test('真实schema19旧事实迁移到20逐列不变，新Record表为空且外键有效', async t => {
...
test('schema20提交前故障整体回滚到19，重试不能丢失旧历史', async t => {
