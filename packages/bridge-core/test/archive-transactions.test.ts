...
test('归档 schema 迁移可回滚；旧账本与执行资产不改写', async t => {
  ...
  const after = new DatabaseSync(f.filePath); try { assert.equal(after.prepare('PRAGMA user_version').get()!.user_version, 18); assert.deepEqual(after.prepare('SELECT * FROM inventory_ledger ORDER BY rowid').all(), ledger); } finally { after.close(); }
});
