...
function facts(filePath: string) {
  ...
  try { return db.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT GLOB 'sqlite_*' AND name NOT GLOB 'recording_plan*' AND name<>'recording_profile_snapshots' ORDER BY name").all().map(({ name }) => [name, db.prepare(`SELECT * FROM ${name} ORDER BY rowid`).all()]); }
  finally { db.close(); }
}
...
test('固定schema17迁移失败完整回滚；重试18逐列保留旧事实', async t => {
