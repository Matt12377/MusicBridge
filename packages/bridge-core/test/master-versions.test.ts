...
test('版本迁移失败回滚既有 schema 与数据，重试成功；新连接读到完全相同的冻结历史', async t => {
  ...
  try {
    const current = new DatabaseSync(f.filePath); try { const schema = current.prepare("SELECT sql FROM sqlite_master WHERE sql IS NOT NULL AND name NOT LIKE '%version%' AND name NOT LIKE 'preparation_%' AND name NOT LIKE 'prepared_%' AND name NOT LIKE 'recording_%' AND name NOT LIKE 'execution_%' AND name NOT LIKE 'sqlite_%' ORDER BY rowid").all(); for (const row of schema) db.exec(String(row.sql)); } finally { current.close(); }
