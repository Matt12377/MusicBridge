...
export interface RecordingFixtureOptions {
  stockQuantities?: { sealedBlank: number; openedBlank: number; legacyUsed: number; unclassified: number };
  afterReceive?: (repository: CollectionRepository) => void;
  /** 容量基准保留自建合成文件；仍关闭全部服务与数据库。 */
...
export async function preparationFixture(t: test.TestContext, options: RecordingFixtureOptions & { probe?: typeof probeReadonlySource; beforeCommit?: (action: string) => void; format?: 'cassette' | 'dat'; emptyB?: boolean } = {}) {
  ...
  repository.receive({ commandId: randomUUID(), model: { brand: 'TDK', name: 'SA', edition: '合成', year: 1990, ...(options.format === 'dat' ? { format: 'dat' as const, tapeType: 'dat' as const } : { format: 'cassette' as const, tapeType: 'II' as const }), identification: 'verified' }, lengthMinutes: 60, quantities: options.stockQuantities ?? { openedBlank: 3, sealedBlank: 0, legacyUsed: 0, unclassified: 0 } });
  options.afterReceive?.(repository);
