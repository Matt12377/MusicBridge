...
test('v1 库存迁移保留全部账本和实体；迁移失败仍保留 v1 可恢复状态', async t => {
  ...
  try { assert.equal(check.prepare('PRAGMA user_version').get()?.user_version, 13); assert.deepEqual(check.prepare('SELECT * FROM inventory_ledger ORDER BY rowid').all(), ledger); }
  finally { check.close(); }
});
...
test('schema 4 草稿迁移中断完整回滚，成功后原音乐与 Roon 关系不变', async t => {
  ...
  const db = new DatabaseSync(filePath); db.exec('DROP TABLE archive_references; DROP TABLE archive_objects; DROP TABLE archive_operations; DROP TABLE archive_roots; DROP TABLE execution_assets; DROP TABLE execution_jobs; DROP TABLE execution_ledger; DROP TABLE recording_sessions; DROP TABLE recording_profile_versions; DROP TABLE recording_profiles; DROP TABLE recording_profile_ledger; DROP TABLE prepared_versions; DROP TABLE prepared_jobs; DROP TABLE prepared_selections; DROP TABLE prepared_ledger; DROP TABLE preparation_workspaces; DROP TABLE preparation_jobs; DROP TABLE preparation_destinations; DROP TABLE preparation_ledger; DROP TABLE layout_versions; DROP TABLE master_versions; DROP TABLE version_jobs; DROP TABLE version_ledger; DROP TABLE media_reservations; DROP TABLE media_plans; DROP TABLE media_ledger; DROP TABLE draft_source_links; DROP TABLE source_bindings; DROP TABLE source_roots; DROP TABLE source_jobs; DROP TABLE source_ledger; DROP TABLE master_drafts; DROP TABLE master_drafts_ledger; PRAGMA user_version=4'); db.close();
