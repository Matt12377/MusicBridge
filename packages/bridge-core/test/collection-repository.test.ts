...
test('v1 库存迁移保留全部账本和实体；迁移失败仍保留 v1 可恢复状态', async t => {
  ...
  try { assert.equal(check.prepare('PRAGMA user_version').get()?.user_version, 17); assert.deepEqual(check.prepare('SELECT * FROM inventory_ledger ORDER BY rowid').all(), ledger); }
  finally { check.close(); }
});
