...
function facts(db: DatabaseSync) { return db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT GLOB 'sqlite_*' AND name NOT GLOB 'collection_progress_*' AND name NOT GLOB 'collection_want*' ORDER BY name").all().map(({ name }) => [name, db.prepare(`SELECT * FROM ${name} ORDER BY rowid`).all()]); }
...
test('固定schema16迁移17逐列保留XLSX原bytes/更正/实体照片/目录历史，迁移失败回滚', async t => {
  ...
  try { assert.equal(db.prepare('PRAGMA user_version').get()?.user_version, 17); assert.deepEqual(facts(db), before); assert.deepEqual(db.prepare('PRAGMA foreign_key_check').all(), []); } finally { db.close(); }
});
