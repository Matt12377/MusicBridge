...
function facts(db: DatabaseSync) {
  return db.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT GLOB 'sqlite_*' AND name NOT GLOB 'recording_attempt*' AND name NOT GLOB 'recording_record*' ORDER BY name").all()
...
test('真实schema18旧事实迁移到20逐列不变，新Attempt表为空且外键有效', async t => {
...
test('schema20提交前故障整体回滚到18，重试不能丢失旧历史', async t => {
  ...
  assert.equal(after.prepare('PRAGMA user_version').get()!.user_version, 20); assert.deepEqual(facts(after), f.before);
});
