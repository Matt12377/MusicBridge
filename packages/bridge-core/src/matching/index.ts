...
export function isCacheableMatchResult(result: Pick<MatchResult, 'evidence'>): boolean {
  return !result.evidence.includes('roon-library-unavailable');
}
...
export function createMatchCache(
  ...
  const boundedMax = Number.isSafeInteger(maxEntries) && maxEntries > 0
    ...
    : MAX_CACHE_ENTRIES;
  const boundedTtlMs = Number.isSafeInteger(ttlMs) && ttlMs > 0
    ? Math.min(ttlMs, DEFAULT_CACHE_TTL_MS)
    : DEFAULT_CACHE_TTL_MS;
  const entries = new Map<string, { result: MatchResult; expiresAt: number }>();
  return {
    get(recording) {
      const entry = entries.get(recording.neteaseTrackId);
      ...
      entries.delete(recording.neteaseTrackId);
      entries.set(recording.neteaseTrackId, entry);
      return entry.result;
    },
    set(recording, result) {
      entries.delete(recording.neteaseTrackId);
      entries.set(recording.neteaseTrackId, {
        result,
