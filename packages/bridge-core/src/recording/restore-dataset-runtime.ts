...
function openRepository(file: string, required: boolean, check: () => void): CollectionRepository {
  ...
  if (exists) {
    ...
    try {
      ...
      if (!Number.isInteger(version) || version < 1 || version > 18 || inspection.prepare('PRAGMA integrity_check').get()?.integrity_check !== 'ok' || inspection.prepare('PRAGMA foreign_key_check').all().length) unavailable();
