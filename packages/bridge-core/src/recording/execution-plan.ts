...
export function planPreparedExecution(master: MasterVersion, layout: LayoutVersion, prep: FrozenPrepared, format: ExecutionFormat, inputs: readonly { assetId: string; input: ExecutionPcmInput }[]): readonly ExecutionRecipe[] {
  ...
  if (conformance.status !== prep.conformance.status || !['MATCHED','ACCEPTED_VARIANCE'].includes(conformance.status) || inputs.length !== prep.assets.length || new Set(inputs.map(i => i.assetId)).size !== inputs.length) return executionFail('VERSION_MISMATCH');
