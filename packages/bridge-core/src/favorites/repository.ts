...
export function createLocalFavoriteRepository(
    ...
    loading = (async () => {
      ...
      try {
        ...
        if (!parsed || typeof parsed !== 'object' || (parsed as { version?: unknown }).version !== 1) {
          throw new TypeError('Unsupported local favorites schema');
        }
        const values = (parsed as { favorites?: unknown }).favorites;
        if (!Array.isArray(values) || values.length > MAX_FAVORITES) {
          throw new TypeError('Invalid local favorites collection');
        }
        const records: FavoriteRecord[] = [];
        for (const value of values) {
          if (!isPersistedRecord(value)) {
            throw new TypeError('Invalid local favorite record');
          }
          records.push(cloneRecord(value));
        }
        favorites.clear();
        for (const record of records) favorites.set(record.favoriteId, record);
        loaded = true;
      } catch (error) {
        ...
        favorites.clear();
        throw new Error('Local favorites file is invalid', { cause: error });
      }
    })();
    try {
      await loading;
  ...
  return {
    ...
    setFavorite(descriptor, favorite) {
      validateDescriptor(descriptor);
      return enqueueMutation(async () => {
        ...
        if (!favorite) {
          ...
          try {
            ...
            if (previous) favorites.set(id, previous);
            throw error;
          }
          return undefined;
        }
