...
- 初版实现提交：`f8635b033d04052b5968aa3657c6c16da67e32cc`；实机问题修复提交：`7c68473e0864c702d206d0ddbc3388d0de2ca99f`。初版报告提交为 `45a443f913e55f430a59021bc96885c92a074b3a`；本次收尾由独立文档提交记录，下一分支基线为本报告最终提交 HEAD。
...
- URL 只存本机 `roon-display.json`，文件权限 0600。Owner 地址已写入正式应用的本机配置，不进入 Git；普通重启后配置恢复、Display 已连接，并在正式安装版收到真实歌词。
- 实机发现 Browse 艺人列表包含制作人员、Display 仅包含主艺人时，初版严格身份核对会拒绝真实歌词。修订只在同 Zone 的独立 Roon Transport 完整佐证标题、专辑、艺人和时长，且主艺人属于 Browse 艺人集合时接受；没有改为模糊匹配。若 Transport 晚于歌词到达，后续观测会重新核对尚未匹配状态。
...
| Core 歌词、匹配、runtime、utility IPC 相关测试 | 最新修订 134/134，0 | 合成，非 Core 全量；含错误身份拒绝与迟到 Transport 恢复 |
...
| 普通安装版启动 | 通过，可见真实媒体库并可播放 | 初版钥匙串等待已解除；修订安装版亦普通启动成功，未绕过系统提示 |
| 真实本地歌曲歌词 | 初版单曲显示；修订版原失败曲目显示 41 行，来源正确，跨行滚动/高亮及暂停恢复通过 | 原生界面观测，不作为完整音频听感验收 |
...
- 修订日志：`mb-display-credits-final-tests.log`、`mb-display-credits-final-types.log`、`mb-display-credits-build.log`；迟到 Transport 测试先失败的证据为 `mb-display-late-identity-red.log`。Contracts/Desktop 测试及连接器 Gate 为未受修订影响的初版轮次，不合并冒充一次全量回归。
- 修订包证据：`musicbridge-lyrics-install-KO4Zdl/package-credits.log`、`credits-packaged-startup.log`、`credits-installed-startup.log`，退出码均为 0。
...
- 最终修订构建包与正式安装包 `Contents/Resources/app.asar` SHA-256 均为 `3683a39e83ef5fe29f451cf071ef2415a32bfe0f2e654d7bb7de1af6409a22b2`，构建来源为 `7c68473`。
- 初版歌词包另完整保存在 `/Volumes/LifeWeave/Developer/CommandLine/Backups/MusicBridge/2026-09-19-lyrics-credits-XocLTa/Music Bridge for Roon.app`；其 SHA-256 为 `748063119c5ca3666aaafad9056ea33849b1529517ff3dedb438973fe7fa5b27`。两个备份均未删除。
- 初版曾因 `SecItemCopyMatching → SecurityServer::decrypt` 等待而受阻。后续普通启动与重启成功，已解除该阻塞；修订包普通启动也进入真实媒体库。未操作 SecurityAgent、删除/重置钥匙串、导出/重新录入 Provider 凭据，也未将 mock keychain 用于真实用户数据。
- 初版真实单曲显示逐行歌词并可暂停；换曲进入 idle 时旧歌词清除。另一个多艺人曲目虽有 Display LRC，但初版显示暂无歌词，由此定位身份差异并修复。
- 修订正式安装版从专辑重新播放该失败曲目，显示 41 行歌词与正确来源；暂停在 0:14 后保持，恢复播放后进度继续，再次暂停成功。最终推进至 1:40 时，截图显示歌词已跨行滚动且高亮改变；留下暂停状态，未改音量。原生截图已查看，未将完整歌词保存到报告或仓库。
- 真实 seek 操作未取得位置变化证据，不标为通过，也不据此判定产品缺陷；整首同步、跨专辑连续播放与 Owner 听感验收仍保留。
